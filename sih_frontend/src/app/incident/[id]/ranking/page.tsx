    ...
    <div className="flex-1 flex flex-col">
      ...
      {isLoading || !ranking ? (
        ...
        <div className="flex flex-col gap-6 flex-1">
          ...
          {/* Top Suspects Podium (Criminal Attribution Leaderboard) */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[rgba(0,90,156,0.12)] pb-4 gap-3">
              <div>
                <div className="flex items-center gap-2 text-[10px] text-[#5A738E] font-bold tracking-wider uppercase mb-1">
                  <span className="inline-block w-2 h-2 rounded-full bg-[#EF3E42] animate-pulse" />
                  <span>PRIMARY FORENSIC INTERCEPT · CRIME SUSPECT PODIUM</span>
                </div>
                <h2 className="font-heading text-xl sm:text-2xl text-[#005A9C] uppercase tracking-wide">
                  Criminal Suspect Attribution Leaderboard
                </h2>
              </div>
              <div className="text-xs text-[#5A738E] flex items-center gap-2 font-mono">
                <span>CLICK ANY SUSPECT PODIUM TO OPEN FORENSIC DOSSIER</span>
              </div>
            </div>

            <div className="py-6 flex items-center justify-center overflow-x-auto">
              <LeaderboardPodium
                rankings={ranking.rows.slice(0, 3).map((r, idx) => ({
                  userId: r.vessel_id,
                  userName: r.name_or_unidentified,
                  rank: r.rank || idx + 1,
                  value: r.confidence_score,
                  isDark: r.is_dark,
                  avatarUrl: getVesselImage(r.vessel_id, r.name_or_unidentified),
                }))}
