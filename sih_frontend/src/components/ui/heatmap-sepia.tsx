...
// ("Heatmap"). Zero dependencies: one WebGL canvas that fills its
// parent. Drop it behind your content with
// <div className="relative"><ShaderBackground className="absolute inset-0" />…
// Adapted from Paper Shaders: https://shaders.paper.design/heatmap
...
vec3 shade(vec2 uv, vec2 p, float t) {
  ...
  for (int i = 0; i < 6; i++) {
    float fi = float(i);
    vec2 center = vec2(
      sin(fi * 2.17 + t * (0.08 + fi * 0.012) + u_seed),
      cos(fi * 1.43 - t * (0.07 + fi * 0.01))
    ) * (0.2 + 0.1 * mod(fi, 3.0));
    field += (0.045 + u_paramA * 0.06) / (dot(p - center, p - center) + 0.018);
  }
  field += (fbm(p * 2.4 + vec2(t * 0.025, -t * 0.02)) - 0.5) * u_intensity;
  float bands = 3.0 + floor(u_intensity * 9.0);
  float contour = floor(clamp(field * 0.18, 0.0, 1.0) * bands) / bands;
  float glow = smoothstep(0.0, mix(0.35, 0.04, u_paramA), abs(fract(field * 0.65) - 0.5));
  return palette(clamp(contour + glow * u_paramA * 0.16, 0.0, 1.0));
}
...
const UNIFORMS = {
  colors: [[0,0,0],[1,0.2980392156862745,0],[1,0.9019607843137255,0.4745098039215686],[0.4196078431372549,0.8431372549019608,1],[0.1843137254901961,0.38823529411764707,0.9058823529411765],[0.06666666666666667,0.12549019607843137,0.41568627450980394],[0.06666666666666667,0.12549019607843137,0.41568627450980394],[0.06666666666666667,0.12549019607843137,0.41568627450980394]] as [number, number, number][],
  colorCount: 6,
  scale: 1.480,
  intensity: 0.520,
  paramA: 0.510,
